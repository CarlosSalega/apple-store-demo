...
function mapDbToProduct(
  dbProduct: Awaited<ReturnType<typeof getProductBySlug>>,
): Product | null {
...
export async function generateMetadata({
  ...
  const imageUrl = primaryImage
    ? resolveImageUrl(primaryImage.url, "product", "detail")
    : null;
  ...
  return {
    title: storage
      ? `${productName} ${storage} - Apple Store`
      : `${productName} - Apple Store`,
    description:
      product?.description ||
      `${productName} ${storage ? storage + " " : ""}${
    ...
    openGraph: {
      ...
      description:
        product?.description ||
        `${productName} - ${product?.condition === "new" ? "Nuevo" : "Usado"} con garantía`,
      type: "website",
      url: `/producto/${slug}`,
      siteName: "Apple Store Demo",
      images: imageUrl
        ? [
            {
              url: imageUrl,
