...
import { z } from "zod";
import { DEMO_SITE_CONFIG } from "@/data/site-config";
...
export async function getSiteConfig(): Promise<SiteConfigData> {
  // ────────────────────────────────────────────────────────────────────────────
  //  MODO DEMO — Los datos se leen del archivo local src/data/site-config.ts
  //  Editá ese archivo para cambiar la info de la landing.
  //  Cuando tengas DB conectada, revertí este cambio y descomentá el bloque
  //  comentado debajo.
  // ────────────────────────────────────────────────────────────────────────────
  ...
  return {
    ...config,
    paymentMethods: String(config.paymentMethods),
    bannerMessages: [...config.bannerMessages],
    storeFeatures: config.storeFeatures.map((f) => ({ ...f })),
  };

  // ═══════════════════════════════════════════════════════════════════════════
  //  Versión DB (cuando tengas PostgreSQL/Neon conectado):
  // ═══════════════════════════════════════════════════════════════════════════
  //
  // import { prisma } from "@/shared/lib/prisma";
  //
  ...
  // let config;
  // try {
  //   config = await prisma.siteConfig.findUnique({ where: { id: "default" } });
  // } catch (error) {
